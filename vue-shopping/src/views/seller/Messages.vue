    ...
    <div v-else class="messages-content">
      ...
      <div class="chat-section" v-if="chatMessages.length > 0">
        <div class="section-title">
          <span>客服消息</span>
          <span class="section-hint">您与用户的对话</span>
        </div>
        <div class="chat-cards">
          ...
          >
            <div class="chat-card" @click="goToChat(chat.id)">
              ...
              <img :src="chat.avatar || defaultAvatar" class="user-avatar" />
              <div class="chat-info">
...
const loadNotifications = async () => {
  try {
    const res = await authAPI.getSellerNotificationsSummary()
...
const goToCard = (type: string) => {
  router.push({ name: 'SellerMessageList', query: { type } })
}
...
const deleteSession = async (chat: any) => {
  try {
    const res = await authAPI.deleteChatSession(chat.id)
    if (res.success) {
...
@import url('@/static/css/seller/消息中心.css');
</style>
