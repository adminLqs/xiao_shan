...
import { ref, computed, onMounted, onActivated, inject, watch, type Ref } from 'vue'
...
const refundId = computed(() => {
  return route.params.refundId as string
})
...
const loadData = async () => {
  ...
  try {
    ...
    } else if (refundStatus !== 'WAITING_RETURN') {
      router.replace({
        name: 'UserLogistics',
        query: { refundId: String(refundId.value), type: 'return' }
      })
...
const submitReturn = async () => {
  ...
  try {
    ...
    if (returnMethod.value === 'SELF') {
      const tn = trackingNumber.value.trim()
      if (!tn) {
        Message.error('请输入物流单号')
        submitting.value = false
        return
      }
      if (!/^[a-zA-Z0-9-]{8,30}$/.test(tn)) {
      ...
      submitData.returnTrackingNumber = tn
    }
    ...
    if (response.success && response.data) {
      ...
      window.dispatchEvent(new CustomEvent('return-submitted', { 
        detail: { refundId: refundId.value } 
      }))
      router.replace({
        name: 'RefundChatStep',
        params: { refundId: String(refundId.value) }
      })
    } else {
