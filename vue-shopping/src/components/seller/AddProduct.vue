...
<script setup lang="ts">
  ...
  const handleFiles = (files: File[]) => {
    ...
    for (const file of files) {
      if (!file.type.match('image/jpeg') && !file.type.match('image/png')) {
        Message.error(`文件 ${file.name} 格式不支持，请上传JPG或PNG格式`)
        continue
      }
  ...
  const submitProductForm = async () => {
    ...
    try {
      ...
      if (response.success) {
        Message.success(response.message || '商品发布成功！')
        resetForm()
      } else {
      ...
      Message.error(error.message || '商品发布失败，请重试')
    } finally {
  ...
  const cancelForm = () => {
    ...
    if (hasUnsavedData) {
      Message.confirm('确定要取消吗？所有未保存的更改将会丢失。', '确认取消').then(() => {
        resetForm()
