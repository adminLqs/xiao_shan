...
<script setup lang="ts">
  ...
  const loadOrderCounts = async () => {
    try {
      ...
      if (response.success && response.data?.counts) {
        ...
        orderCounts.value = {
          total: (counts.PENDING || 0) + (counts.PAID || 0) + (counts.PROCESSING || 0) + 
                (counts.SHIPPED || 0) + (counts.COMPLETED || 0) + (counts.CANCELLED || 0) + (counts.REFUNDED || 0),
        ...
        pendingOrdersCount.value = (counts.PENDING || 0) + (counts.SHIPPED || 0)
      }
    } catch (error: any) {
  ...
  const loadRecentOrders = async () => {
    try {
      const response = await authAPI.getOrders({ page: 1, pageSize: 3 })
  ...
  const loadWishlist = async () => {
    try {
      const response = await authAPI.getFavorites({ page: 1, pageSize: 4 })
      if (response.success && response.data?.records) {
        wishlist.value = response.data.records
      }
    } catch (error: any) {
  ...
  const handlePay = async (orderId: number) => {
    try {
      const loading = Message.loading({ message: '正在获取支付信息...' })
      ...
      if (response.success && response.data?.paymentHtml) {
        ...
        Message.error(response.message || '获取支付信息失败')
      }
    } catch (error: any) {
  ...
  const addToCart = async (productId: number) => {
    try {
      const loading = Message.loading({ message: '添加中...' })
  ...
  const removeFromWishlist = async (favoriteId: number) => {
    try {
      await Message.confirm('确定要从收藏中移除该商品吗？', '移除收藏')
