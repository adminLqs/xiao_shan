<template>
  <div class="address-selector address-selector-wrapper">
    <div class="region-picker">
      <div class="picker-tabs">
        <div
          class="picker-tab"
          :class="{ active: currentTab === 'province' }"
          @click="currentTab = 'province'"
        >
          {{ selectedProvince || '请选择省份' }}
          <span class="arrow" v-if="currentTab !== 'province'">›</span>
        </div>
        <div
          class="picker-tab"
          :class="{ active: currentTab === 'city', disabled: !selectedProvince }"
          @click="selectedProvince && (currentTab = 'city')"
        >
          {{ selectedCity || '请选择城市' }}
          <span class="arrow" v-if="currentTab !== 'city' && selectedCity">›</span>
        </div>
        <div
          class="picker-tab"
          :class="{ active: currentTab === 'district', disabled: !selectedCity }"
          @click="selectedCity && (currentTab = 'district')"
        >
      ...
      <div class="picker-content">
        <div v-show="currentTab === 'province'" class="picker-list">
          <div
          ...
          >
            <span>{{ province }}</span>
            <i v-if="selectedProvince === province" class="fas fa-check"></i>
          </div>
        </div>

        <div v-show="currentTab === 'city'" class="picker-list">
          <div
          ...
          >
            <span>{{ city }}</span>
            <i v-if="selectedCity === city" class="fas fa-check"></i>
          </div>
        </div>

        <div v-show="currentTab === 'district'" class="picker-list">
          <div
            v-for="district in districts"
            :key="district"
            class="picker-item"
            :class="{ selected: selectedDistrict === district }"
            @click="selectDistrict(district)"
          >
            <span>{{ district }}</span>
            <i v-if="selectedDistrict === district" class="fas fa-check"></i>
          </div>
        </div>
      </div>
    </div>
...
const selectedProvince = ref(props.province || props.modelValue?.province || '')
const selectedCity = ref(props.city || props.modelValue?.city || '')
const selectedDistrict = ref(props.district || props.modelValue?.district || '')

const regionData: Record<string, Record<string, string[]>> = {
  '北京市': { '市辖区': ['东城区', '西城区', '朝阳区', '丰台区', '石景山区', '海淀区', '门头沟区', '房山区', '通州区', '顺义区', '昌平区', '大兴区', '怀柔区', '平谷区', '密云区', '延庆区'] },
  '天津市': { '市辖区': ['和平区', '河东区', '河西区', '南开区', '河北区', '红桥区', '东丽区', '西青区', '津南区', '北辰区', '武清区', '宝坻区', '滨海新区', '蓟州区', '宁河区'] },
  '上海市': { '市辖区': ['黄浦区', '徐汇区', '长宁区', '静安区', '普陀区', '虹口区', '杨浦区', '闵行区', '宝山区', '嘉定区', '浦东新区', '金山区', '松江区', '青浦区', '奉贤区', '崇明区'] },
  '重庆市': { '市辖区': ['万州区', '涪陵区', '渝中区', '大渡口区', '江北区', '沙坪坝区', '九龙坡区', '南岸区', '北碚区', '渝北区', '巴南区', '黔江区', '长寿区', '江津区', '合川区', '永川区', '南川区', '璧山区', '铜梁区', '潼南区', '荣昌区', '开州区', '梁平区', '武隆区'] },
  '河北省': {
  ...
  '江苏省': {
    ...
    '苏州市': ['姑苏区', '虎丘区', '吴中区', '相城区', '苏州工业园区', '吴江区', '常熟市', '张家港市', '昆山市', '太仓市'],
  ...
  '安徽省': {
    ...
    '芜湖市': ['镜湖区', '弋江区', '鸠江区', '三山区', '繁昌县', '南陵县', '无为市'],
  ...
  '河南省': {
    ...
    '洛阳市': ['老城区', '西工区', '瀍河回族区', '涧西区', '吉利区', '洛龙区', '偃师区', '孟津区', '新安县', '栾川县', '嵩县', '汝阳县', '宜阳县', '洛宁县', '伊川县'],
    ...
    '周口市': ['川汇区', '淮阳区', '扶沟县', '西华县', '商水县', '沈丘县', '太康县', '鹿邑县', '项城市'],
    '驻马店市': ['驿城区', '西平县', '上蔡县', '平舆县', '正阳县', '确山县', '泌阳县', '汝南县', '遂平县', '新蔡县'],
    '济源市': ['沁园街道', '济水街道', '北海街道', '天坛街道', '玉泉街道', '梨林镇', '五龙口镇', '思礼镇', '承留镇', '大峪镇']
  },
  '湖北省': {
    ...
    '荆州市': ['沙市区', '荆州区', '公安县', '江陵县', '石首市', '洪湖市', '松滋市'],
    '黄冈市': ['黄州区', '团风县', '红安县', '罗田县', '英山县', '浠水县', '蕲春县', '黄梅县', '麻城市', '武穴市'],
    '咸宁市': ['咸安区', '嘉鱼县', '通城县', '崇阳县', '通山县', '赤壁市'],
    '随州市': ['曾都区', '随县', '广水市'],
    '恩施土家族苗族自治州': ['恩施市', '利川市', '建始县', '巴东县', '宣恩县', '咸丰县', '来凤县', '鹤峰县'],
    '仙桃市': ['干河街道', '龙华山街道', '沙嘴街道', '郑场镇', '毛嘴镇', '豆河镇', '三伏潭镇', '胡场镇', '长埫口镇', '彭场镇', '沙湖镇', '杨林尾镇'],
    '潜江市': ['园林街道', '杨市街道', '周矶街道', '广华街道', '泰丰街道', '高石碑镇', '积玉口镇', '渔洋镇', '王场镇', '高场镇', '熊口镇', '老新镇', '浩口镇'],
    '天门市': ['竟陵街道', '侨乡街道', '杨林街道', '多宝镇', '拖市镇', '张港镇', '蒋场镇', '侯口镇', '汪场镇', '渔薪镇', '黄潭镇', '岳口镇']
  },
  '湖南省': {
  ...
  '广东省': {
    ...
    '东莞市': ['莞城街道', '石龙镇', '虎门镇', '东城街道', '万江街道', '南城街道', '中堂镇', '望牛墩镇', '麻涌镇', '石碣镇', '高埗镇', '道滘镇', '洪梅镇', '沙田镇', '厚街镇', '长安镇', '寮步镇', '大岭山镇', '大朗镇', '黄江镇', '樟木头镇', '凤岗镇', '塘厦镇', '谢岗镇', '清溪镇', '常平镇', '桥头镇', '横沥镇', '东坑镇', '企石镇', '石排镇', '茶山镇'],
    '中山市': ['石岐街道', '东区街道', '中山港街道', '西区街道', '南区街道', '五桂山街道', '小榄镇', '黄圃镇', '民众镇', '东凤镇', '古镇镇', '沙溪镇', '坦洲镇', '港口镇', '三角镇', '横栏镇', '南头镇', '阜沙镇', '南朗街道', '三乡镇', '板芙镇', '大涌镇', '神湾镇'],
    '潮州市': ['湘桥区', '潮安区', '饶平县'],
  ...
  '海南省': {
    ...
    '三亚市': ['海棠区', '吉阳区', '天涯区', '崖州区'],
    '三沙市': ['西沙群岛', '南沙群岛', '中沙群岛'],
    '儋州市': ['那大镇', '和庆镇', '南丰镇', '大成镇', '雅星镇', '兰洋镇', '光村镇', '海头镇', '木棠镇', '峨蔓镇', '王五镇', '白马井镇', '中和镇', '排浦镇', '东成镇', '新州镇'],
    '五指山市': ['通什镇', '南圣镇', '毛阳镇', '番阳镇', '畅好乡', '毛道乡', '水满乡'],
    '琼海市': ['嘉积镇', '万泉镇', '石壁镇', '中原镇', '博鳌镇', '阳江镇', '龙江镇', '潭门镇', '塔洋镇', '长坡镇', '大路镇', '会山镇'],
    '文昌市': ['文城镇', '重兴镇', '蓬莱镇', '会文镇', '东路镇', '潭牛镇', '抱罗镇', '冯坡镇', '锦山镇', '铺前镇', '翁田镇', '昌洒镇', '龙楼镇', '东郊镇', '文教镇', '东阁镇', '新桥镇', '清澜镇'],
  ...
  '四川省': {
    ...
    '资阳市': ['雁江区', '安岳县', '乐至县'],
    '阿坝藏族羌族自治州': ['马尔康市', '汶川县', '理县', '茂县', '松潘县', '九寨沟县', '若尔盖县', '红原县', '壤塘县', '阿坝县'],
    '甘孜藏族自治州': ['康定市', '泸定县', '丹巴县', '九龙县', '雅江县', '道孚县', '理塘县', '巴塘县', '乡城县', '稻城县', '得荣县', '甘孜县', '新龙县', '德格县', '白玉县', '石渠县', '色达县'],
    '凉山彝族自治州': ['西昌市', '德昌县', '会理县', '会东县', '宁南县', '普格县', '布拖县', '金阳县', '昭觉县', '喜德县', '冕宁县', '越西县', '甘洛县', '美姑县', '雷波县', '木里藏族自治县']
  },
  '贵州省': {
    ...
    '黔东南苗族�族自治州': ['凯里市', '黄平县', '施秉县', '三穗县', '镇远县', '岑巩县', '天柱县', '锦屏县', '剑河县', '台江县', '黎平县', '榕江县', '从江县', '雷山县', '麻江县', '丹寨县'],
  ...
  '云南省': {
    ...
    '红河哈尼族彝族自治州': ['个旧市', '开远市', '蒙自市', '弥勒市', '屏边苗族自治县', '建水县', '石屏县', '泸西县', '元阳县', '红河县', '绿春县', '河口瑶族自治县'],
  ...
  '陕西省': {
    ...
    '咸阳市': ['秦都区', '渭城区', '杨陵区', '三原县', '泾阳县', '乾县', '礼泉县', '永寿县', '长武县', '旬邑县', '淳化县', '武功县', '彬州市', '兴平市'],
  ...
  '香港特别行政区': { '香港': ['中西区', '湾仔区', '东区', '南区', '油尖旺区', '深水埗区', '九龙城区', '黄大仙区', '观塘区'] },
  '澳门特别行政区': { '澳门': ['花地玛堂区', '圣安多尼堂区', '望德堂区', '大堂区', '风顺堂区'] },
  '台湾省': {
    '台北市': ['松山区', '信义区', '大安区', '中山区', '中正区', '大同区', '万华区', '文山区', '南港区', '北投区'],
    '新北市': ['板桥区', '三重区', '中和区', '永和区', '土城区', '树林区', '莺歌区', '三峡区', '淡水区', '汐止区', '瑞芳区', '新店区', '深坑区', '石碇区', '坪林区', '平溪区', '双溪区', '贡寮区', '金山区', '万里区', '乌来区', '泰山区', '林口区', '八德区'],
    '桃园市': ['桃园区', '中坜区', '平镇区', '八德区', '杨梅区', '芦竹区', '大溪区', '龙潭区', '龟山区', '大园区', '观音区', '新屋区', '复兴区'],
    '台中市': ['中区', '东区', '南区', '西区', '北区', '北屯区', '西屯区', '南屯区', '太平区', '大里区', '雾峰区', '乌日区', '丰原区', '后里区', '石冈区', '新社区', '潭子区', '大雅区', '神冈区', '大肚区', '沙鹿区', '龙井区', '梧栖区', '清水区', '大甲区', '外埔区', '大安区'],
    '台南市': ['东区', '南区', '北区', '中西区', '安平区', '安南区', '永康区', '归仁区', '新化区', '左镇区', '玉井区', '楠西区', '南化区', '仁德区', '关庙区', '龙崎区', '官田区', '麻豆区', '佳里区', '西港区', '七股区', '将军区', '学甲区', '北门区', '新营区', '盐水区', '白河区', '柳营区', '后壁区', '东山区', '六甲区', '下营区', '大内区', '山上区', '新市区', '安定区'],
...
watch(() => props.province, (newVal) => {
  if (newVal !== undefined && newVal !== selectedProvince.value) {
    selectedProvince.value = newVal
  }
})

watch(() => props.city, (newVal) => {
  if (newVal !== undefined && newVal !== selectedCity.value) {
    selectedCity.value = newVal
  }
})

watch(() => props.district, (newVal) => {
  if (newVal !== undefined && newVal !== selectedDistrict.value) {
...
.region-picker:focus-within {
  border-color: #4a6491;
  background: white;
  box-shadow: 0 0 0 3px rgba(74, 100, 145, 0.08);
}
...
.picker-tab.active::after {
  ...
  width: 40px;
  height: 3px;
  background: linear-gradient(135deg, #4a6491, #6b8ab5);
  border-radius: 2px;
}
...
.address-selector-wrapper:focus-within {
  border-color: #4a6491;
  background: white;
  box-shadow: 0 0 0 3px rgba(74, 100, 145, 0.08);
}

.address-selector-wrapper :deep(.region-picker) {
  border-radius: 12px;
  border: none;
